...
namespace calice_eudaq {
  ...
  {
    //cerr << "CaliceGenericConverterPlugin::GetLCIOSubEvent" << endl;
    ...
    if(rawev->GetSubType() != EVENT_TYPE){
      cerr << "CaliceGenericConverter: type failed!" << endl;
      return false;
    }
    ...
    //cerr << "nblock = " << rawev->NumBlocks() << endl;
    ...
    if(bl3.size()<8){cerr << "CaliceGenericConverter: time data too short!" << endl; return false;}
        ...
        for(unsigned int i=0;i<vec.size()-2;i+=3){
          ...
          if(output.size() == 10){
            ...
            try{
              ...
              cerr << "CaliceGenericConverterPlugin: the collection to add is read only! skipped..." << endl;
              delete obj;
            }
            output.clear();
          }
    ...
    //cerr << nblock-2 << " elements added to " << colName << endl;
