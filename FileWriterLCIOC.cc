...
namespace eudaq {
  ...
  void FileWriterLCIOC::WriteEvent(const DetectorEvent & devent) {
    ...
    std::cout << "FileWriterLCIOC: Send data to network." << std::endl;
    ...
    std::cout << "EUDAQ_DEBUG: FileWriterLCIOC::WriteEvent() processing event "
    ...
    if (!lcevent->getCollectionNames()->empty()) {
      // std::cout << " FileWriterLCIOC::WriteEvent() : doing the actual writing : " <<std::flush;
